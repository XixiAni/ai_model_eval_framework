...
基于Python+Requests开发的企业级AI大模型接口自动化测试框架，采用分层解耦+数据驱动设计，实现用例与代码完全分离，支持批量自动执行、智能断言、多维统计与CSV报告导出，适用于大模型API自动化评测、回归测试、性能监控场景。
...
- CSV报告： ./report/eval_result.csv （Excel直接打开）
- 全链路日志： ./api_test.log 
...
2.6 result_exporter.py（CSV导出工具）
...
2.7 chat_cases_new.yaml（数据驱动用例）
